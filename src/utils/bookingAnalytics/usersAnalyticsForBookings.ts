...
import { UserBookingsSourcesShort } from "../../enums/BookingSource";
import Hotel from "../../models/hotels/hotelModel";
import util from "util"


const userAnalyticsForBookings = async (
    startDay: string,
    days: number,
    isTable: boolean,
    ...
    let aggregation = [
        ...
        },
        {
            $project: {
                ...
                dob: "$userId.dob",
                isCreatedByManager: {
                    $toBool: "$userId.isCreatedByManager"
                },
                isSameTime: {
                    $cond: {
                        if: {
                ...
                _id: {
                    ...
                    bookingSource: {
                        $cond: {
                            ...
                            then: {
                                $cond: ["$isSameTime", "OXY", "OOXY"]
                            },
                            else: { $cond: ["$isSameTime", "USER", "OUSER"] }
                        }
                    },
                },
                bookings: {
    ...
    for (let i = 0; i < hotels.length; i++) {
        ...
        for (let j = 0; j < allDays.length; j++) {
            ...
            for (let k = 0; k < UserBookingsSourcesShort.length; k++) {
                let thatData = undefined
                try {
                    thatData = data[0].data[hotels[i]._id.toString()][allDays[j]][UserBookingsSourcesShort[k]]
                } catch (err) {
                }
                if (thatData) {
                    thatDateTypeData[UserBookingsSourcesShort[k]] = thatData
                } else {
                    thatDateTypeData[UserBookingsSourcesShort[k]] = isTable ? 0 : []
                }
            }
...
export default userAnalyticsForBookings
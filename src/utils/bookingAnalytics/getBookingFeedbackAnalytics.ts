...
import { FeedbackCountAnalyticsShort } from "../../enums/BookingSource";
import Hotel from "../../models/hotels/hotelModel";

const getBookingFeedbackAnalytics = async (
  startDay: string,
  days: number,
  isTable: boolean,
  ...
  let earliestCheckInEndTime = moment
    .tz(`${startDay} 12:00:00`, "DD-MM-YYYY HH:mm:ss", "Asia/Kolkata")
    .add(days, "day")
    .add(12, "hour");
  let latestCheckOutStartDate = moment
    .tz(`${startDay} 11:00:00`, "DD-MM-YYYY HH:mm:ss", "Asia/Kolkata")
    .subtract(12, "hour");
      ...
      $group: {
        ...
        bookings: {
          $push: {
            ...
            amount: "$amount",
            status: {
              $cond: ["$hasCheckedOut", "Checked Out", "Checked In"],
            },
              ...
              $cond: [
                {
                  $eq: [
                    {
                      $substrBytes: ["$fCheckIn", 2, 10],
                    },
                    {
                      $substrBytes: ["$fCheckOut", 2, 10],
                    },
                  ],
                },
                {
                  ...
                  $cond: [
                    {
                      $eq: [
                        {
                          $substrBytes: ["$fCheckIn", 6, 10],
                        },
                        {
                          $substrBytes: ["$fCheckOut", 6, 10],
                        },
                      ],
                    },
                    {
  ...
  for (let i = 0; i < hotels.length; i++) {
    ...
    for (let j = 0; j < allDays.length; j++) {
      ...
      for (let k = 0; k < FeedbackCountAnalyticsShort.length; k++) {
        let thatData = undefined;
        try {
          thatData =
            data[0].data[hotels[i]._id.toString()][allDays[j]][
              FeedbackCountAnalyticsShort[k]
            ];
        } catch (err) {}
        if (thatData) {
          thatDateTypeData[FeedbackCountAnalyticsShort[k]] = thatData;
        } else {
          thatDateTypeData[FeedbackCountAnalyticsShort[k]] = isTable ? 0 : [];
        }
      }
...
export default getBookingFeedbackAnalytics;

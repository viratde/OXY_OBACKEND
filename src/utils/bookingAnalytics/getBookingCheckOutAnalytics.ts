...
import { CheckOutSourcesShort } from "../../enums/BookingSource";
import Hotel from "../../models/hotels/hotelModel";


const getBookingCheckOutAnalytics = async (
    startDay: string,
    days: number,
    isTable: boolean,
    ...
    for (let i = 0; i < hotels.length; i++) {
        ...
        for (let j = 0; j < allDays.length; j++) {
            ...
            for (let k = 0; k < CheckOutSourcesShort.length; k++) {
                let thatData = undefined
                try {
                    thatData = data[0].data[hotels[i]._id.toString()][allDays[j]][CheckOutSourcesShort[k]]
                } catch (err) {
                }
                if (thatData) {
                    thatDateTypeData[CheckOutSourcesShort[k]] = thatData
                } else {
                    thatDateTypeData[CheckOutSourcesShort[k]] = isTable ? 0 : []
                }
            }
...
export default getBookingCheckOutAnalytics
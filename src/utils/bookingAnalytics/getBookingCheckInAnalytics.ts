...
import { CheckInSourcesShort } from "../../enums/BookingSource";
import Hotel from "../../models/hotels/hotelModel";


const getBookingCheckInAnalytics = async (
    startDay: string,
    days: number,
    isTable: boolean,
    ...
    for (let i = 0; i < hotels.length; i++) {
        ...
        for (let j = 0; j < allDays.length; j++) {
            ...
            for (let k = 0; k < CheckInSourcesShort.length; k++) {
                let thatData = undefined
                try {
                    thatData = data[0].data[hotels[i]._id.toString()][allDays[j]][CheckInSourcesShort[k]]
                } catch (err) {
                }
                if (thatData) {
                    thatDateTypeData[CheckInSourcesShort[k]] = thatData
                } else {
                    thatDateTypeData[CheckInSourcesShort[k]] = isTable ? 0 : []
                }
            }
...
export default getBookingCheckInAnalytics
...
const managerGetPendingPaymentBookings = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    let data = [
      ...(bookings.map((book) => managerBookingFormatter(book,BookingTypeFormatterHelper.Pending))),
    ]
...
export default managerGetPendingPaymentBookings;

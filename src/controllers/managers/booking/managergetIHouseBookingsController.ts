...
const managerGetIHouseBookings = async (
  req: Request,
  res: Response
): Promise<Response> => {
  try {
    ...
    return res.status(200).json({
      ...
      data: bookings.map(book => managerBookingFormatter(book,BookingTypeFormatterHelper.InHouse)),
    });
  } catch (err) {
    console.log(err);
    return res
...
export default managerGetIHouseBookings;

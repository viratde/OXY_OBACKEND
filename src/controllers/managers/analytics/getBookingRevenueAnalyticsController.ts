...
const managerGetBookingRevenueAnalyticsController = async (
    req: Request,
    res: Response
): Promise<Response> => {
    ...
    try {
        ...
        const data = await getBookingRevenueAnalytics(startDay, days, isTable, hotelIds.map(hot => new Types.ObjectId(hot)))
...
export default managerGetBookingRevenueAnalyticsController
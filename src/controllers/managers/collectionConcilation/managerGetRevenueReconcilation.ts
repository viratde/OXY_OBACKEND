...
import collectionConcilationWrapper from "../../../utils/collectionConcilation/collectionConcilationWrapper";

const managerGetCollectionConcilation = async (req: Request, res: Response): Promise<Response> => {
    try {
        ...
        let data = await collectionConcilationWrapper(startDay,days,hotelIds.map(id => new Types.ObjectId(id)),isTable)
...
export default managerGetCollectionConcilation
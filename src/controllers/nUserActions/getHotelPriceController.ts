...
import getHotelPrice from "../../utils/nUserActions/getHotelPrice"


const getHotelPriceController = async (
    req:Request,
    res:Response
) => {
    ...
    try {
        ...
        const { guests, checkIn, checkOut,roomType } = req.query
        ...
        if (!roomType) {
            return res.status(400).json({
                status: false,
                message: "Please choose correct room type"
            })
        }
        ...
        return res.status(200).json({
            ...
            data: await getHotelPrice(aGuest, (checkInDate ? checkInDate : minCheckInDate), (checkOutDate && checkInDate ? checkOutDate : minCheckOutDate), new Types.ObjectId(hotelId),roomType as string)
        })
...
export default getHotelPriceController
